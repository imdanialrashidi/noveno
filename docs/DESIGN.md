...
- **Header:** hairline bottom border; logo mark + English-only «NOVENO» wordmark (Estedad 800, +0.04em tracking — accepted 2026-08-14, §17); nav links Estedad 500 14px; primary CTA button. Desktop: not sticky (calm; the CTA recurs in sections). **Mobile: compact sticky header with the audit CTA always visible** (Spec §49); menu trigger ≤ 44px, no mega-menu (Spec §9.2).
...
| 2026-08-14 | **English-only wordmark (accepted):** header/footer logo lockup renders the mark + the Estedad «NOVENO» wordmark only (no stacked Persian wordmark); mark fill = semantic `text` token via `currentColor` (theme-compatible in both themes); Persian «نوونو» remains the brand in copy, `<title>`s, and the logo link aria-label («نوونو — صفحه اصلی»). Resolves §5.2/§9 (Latin wordmark in Estedad, +0.04em tracking) and supersedes the earlier «نوونو» wordmark line in §8. Lockup scale: header mark 36px / wordmark 16px, footer 44px / 16px; the mark keeps a non-zero width floor at 320px during fallback-font rendering (flex `min-w`). | Commits e6cda4d + add6fe2; independent review at add6fe2; 320px fallback-font browser re-check (fonts blocked) | Revisit when the founder wants the Persian wordmark restored → design review |
