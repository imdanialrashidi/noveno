...
Status: active — **Slice 1 complete (2026-08-11)**; **Slice 2 complete (2026-08-11)**; **visual redesign complete (2026-08-14, founder-directed)**; **email-only architecture complete (2026-10)**; launch awaiting founder provisioning + deploy.
Updated: 2026-10

> **Status (2026-10, architecture change):** **Supabase has been removed entirely.** The founder decided v1 needs no persistent lead store. The audit function (`/api/audit`) now **validates only** (returns `{ ok: true, status: "validated" }`); **Web3Forms email delivery is the sole lead destination and the only completion gate** — the visitor reaches `/audit/thank-you` only after Web3Forms confirms `{ success: true }`; `audit_submitted`, the draft clear, and the done marker all fire only then. There is no database, no migration, no `SUPABASE_*` env, no `@supabase/supabase-js`, no `supabase/` directory, and no backup step. Setup is Turnstile keys + Web3Forms access key + Pages project only (`docs/ops/setup-checklist.md`).
...
- **Delivered:** `/audit` + `/audit/thank-you` on the Slice 1 design system (six stations, StepperLine client-driven states, chips with RTL arrow-key navigation, per-field/step/form validation, submitting state, retry banner with values preserved, offline banner with contact fallback, aria-live progress + announcements, Turnstile explicit render in the contact station with theme sync + reset-on-retry); framework-free state machine (`src/scripts/audit.ts`: sessionStorage draft `noveno:audit:draft`, stable `submission_id`, attribution from the session capture `noveno:attribution`); `functions/api/audit.ts` trust boundary (size limit → JSON → honeypot → whitelist validation + Persian-digit normalization → per-isolate rate limiter → Turnstile siteverify with remoteip + idempotency_key → 200 `validated` / 400/403/429/413/405/500); `functions/api/events.ts` + `src/scripts/analytics.ts` (queue + sendBeacon, declarative `data-event` wiring, Analytics Engine `NOVENO_EVENTS`, 501-degraded, no PII); Web3Forms client delivery as the completion gate (bounded two attempts, inspected `{ success: true }` response, truthful failure banner, never a false thank-you); **Supabase removed (2026-10)**; `CTA_URL` flipped to `/audit`; sitemap/robots/_headers/JSON-LD/og.png (1200×630); `.env.example` names-only extension; docs/ops setup checklist + runbook; `scripts/slice2-test-server.mjs` deterministic test server (mock Web3Forms + recording Analytics Engine).
- **Verification:** `npm run check` 0 errors; `npm test` 186/186 (incl. audit-function trust-boundary tests: normalization, enums, rate limiter, Turnstile fake+REAL siteverify with official test keys against challenges.cloudflare.com, request-level invariant ordering, validate-only response contract, events endpoint incl. flood rate-limit, sitemap/robots/headers/JSON-LD/og.png/honeypot/guard-script, CTA flip, client↔server enum drift guard; audit-retry journey tests: Web3Forms success gate, delivery-failure truthfulness, preserved values + stable submission_id, fresh Turnstile tokens, bounded retries, no token leakage to Web3Forms); `bash scripts/verify.sh` exit 0 (pi-doctor + project contract green) from a clean checkout; browser matrix at 1440/390/320 + both themes: full journey ×4 (incl. one with UTM attribution → lead email), reload-mid-journey restore, validation states (incl. aria-describedby linkage), network-failure banner + retry with values preserved, offline banner, Turnstile rejection (always-fail secret → 403), Web3Forms failure (2 attempts, truthful banner, **no thank-you**, values preserved), Web3Forms success → thank-you only then, thank-you guard (submit path hidden pre-paint + direct-visit visible, CLS ≤ 0.0003 both paths), keyboard/aria snapshot — screenshots under `.artifacts/playwright/slice2/`.
...
- **UNPROVEN (external, no credentials in this environment):** real Web3Forms email delivery (mock endpoint exercised; response inspection contract tested); deployed Analytics Engine writes (binding unavailable locally; 501-degraded path proven); appearance-dependent craft criteria (no image vision — screenshots archived for human review); field CWV (lab only).
...
- **A4 — Trust invariants hold (email-only).** (i) A successful/thank-you state is only ever presented after Web3Forms confirms delivery (`{ success: true }`); (ii) server validation success alone is never presented as success — the form/draft stays intact until delivery confirms; (iii) a failed delivery is always surfaced truthfully (banner + retry + direct contact), never as success. — proof: defect-sensitive function tests + client journey tests + staging integration exercising simulated Web3Forms failure.
- **A5 — Duplicate-submission honesty.** Without a durable store, exactly-once delivery is impossible by design. The stable `submission_id` rides in the email so duplicates are recognizable; the client prevents duplicate sends within one in-flight interaction. — proof: journey tests (stable id across retries) + runbook note documenting the accepted trade-off.
- **A6 — Attribution preserved with the lead, independent of analytics.** UTM/referrer/landing page/first-seen ride in the Web3Forms lead email. — proof: delivery-payload test asserting attribution fields + staging submission with crafted UTM params → email inspection; analytics outage simulation does not affect the payload.
...
- **Data:** `src/data/site.ts` — typed constants: nav, contact facts (09353598620, imdanialrashidi@gmail.com, Instagram @noveno_ir, WhatsApp/Telegram), three offers, FAQ, process stages, audit stations, proof entries pointer, **`CTA_URL`** (Slice 1 → `/contact`; Slice 2 flips to `/audit`), **`HERO_HEADLINE`** (one of the two approved candidates — single-line swap).
...
- Submit flow: disable + inline progress («در حال ارسال…») → POST `/api/audit` → on 200 `validated`: browser posts the sanitized lead to **Web3Forms** and inspects the response (`{ success: true }`, bounded two attempts) → **only on confirmed delivery**: fire `audit_submitted`, clear the draft, record the done marker, navigate to `/audit/thank-you`. On recoverable errors (validation/network/turnstile/delivery) → truthful banner + «تلاش دوباره», **field values preserved**, draft intact; offline (`navigator.onLine`) shows the same banner with the contact fallback emphasized (DESIGN §11). No false success: server validation alone never completes the journey.
...
200 { ok: true, status: "validated" }             // validation accepted — NOT a delivery signal
...
- Environment (names only, in `.env.example`):
  ...
  - Pages secret (never in repo): `TURNSTILE_SECRET_KEY`.
...
Visitor ──▶ Static Astro site (fa/RTL, theme)                          [Slice 1]
        ...
        POST /api/audit ──▶ Pages Function (only server boundary)      [Slice 2]
              ...
              ▼  200 { ok: true, status: "validated" }
        browser → Web3Forms POST (bounded 2 attempts, inspected { success: true })
...
**In scope.** `/audit` page (DESIGN §11: two-column desktop with StepperLine rail, single-column mobile, 6 stations, MultiSelect chips with keyboard navigation, per-field + step + form validation, submitting state, error banner + retry with values preserved, offline banner, progress announce via aria-live); `/audit/thank-you` (extended JourneyLine, dashed future stations «بررسی ← تماس ← گفتگو ← پیشنهاد», «شما اینجا هستید» marker, NextStepsRail, contact fallback, optional pointer to a real case study); `src/scripts/audit.ts` (state machine, sessionStorage draft, attribution capture, submission_id, Turnstile explicit render, submit/retry semantics, Web3Forms delivery gate); `functions/api/audit.ts` (validate-only trust boundary per §5.5) with unit tests; `functions/api/events.ts` + `src/scripts/analytics.ts` (event model, Analytics Engine, non-blocking, no PII); `_headers` (fonts immutable + security headers); sitemap.xml, robots.txt, structured data (Organization/Service), metadata completion; `.env.example` extension + Pages setup checklist + runbook (no deployment executed); lab performance measurement (CWV targets per QUALITY), final security/a11y hardening; `CTA_URL` flip to `/audit`.
...
- **R1 — Web3Forms free-tier constraint (resolved).** Official docs: server-side usage requires paid plan + server-IP whitelisting (impractical for Cloudflare egress). Resolution: client-side delivery as the completion gate (§5.6); email is the lead record. Residual: delivery depends on the browser session → mitigated by bounded await + one automatic retry + truthful failure banner with direct contact; no false success. Do not silently "fix" by switching delivery providers — requires explicit approval.
...
- **R3 — REMOVED (2026-10):** supabase-js no longer exists in the stack; `nodejs_compat` stays for the Turnstile/analytics fetch surface.
...
- **R7 — Duplicate/race (2026-10: accepted trade-off).** With no durable store there is no unique-constraint collapse. The client prevents duplicate sends within one in-flight interaction; a retry after an ambiguous delivery failure can theoretically produce a second email — recognized via the stable `submission_id` (runbook). Never rebuild a database to solve this.
- **R8 — No data-integrity surface (2026-10).** With email-only delivery there is no database, no migration, and no backup step. Remaining integrity concern: a retry after an ambiguous delivery failure can produce a duplicate email — recognized via the stable `submission_id` (runbook documents the accepted trade-off).
...
**Current state.** See the status note above — both slices are committed and verified; the app exists (`src/`, `functions/`, `tests/`) and `.pi/verification.json` has an `app` route. Launch awaits founder provisioning (`docs/ops/setup-checklist.md`) and a preview-deploy smoke. CI runs `scripts/verify.sh` + package integrity.
